...
1. **Stage 1** (RST 38h entry, 0x015B–0x016D): reads CLA (port 0x00).  If bit7=0 (regular key), stores to 0x457E (syscall 0x0E) and returns early.  If bit7=1, stores `CLA & 0x7F` to 0x4580 (GETFON register) and falls into Stage 2.  In the emulator model, the direct `0x457E` byte is then consumed when the `0x0516/0x0519` accessor reads it, so helper-side polls do not keep seeing a stale Stage 1 key forever.
