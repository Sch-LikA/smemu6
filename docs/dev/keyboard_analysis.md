...
### Confirmed hardware latch topology (from schematic, November 1978 — J. Zahn)
...
**FOUND latch (4013 FF2, pin 13 = Q):**
- **SET** by the STROBE signal from the S471 encoder (pin 12 = clock) — fires when a key is detected.
- **RESET** (pin 10) is directly connected to **ARRIVE**.

**ARRIVE** is generated by the 4024 counter driven by the 300 KHz oscillator. With a 7-stage
divider it fires at approximately **2.3 KHz** (~every 0.4 ms), which is **~46× per 50 Hz ISR frame**.
ARRIVE marks the start of each new keyboard scan cycle and automatically clears FOUND.

**Critical consequence:** FOUND is cleared by **hardware timing (ARRIVE)**, not by:
- Reading CLA (port 0x00 `IN`) — CLA read does **not** clear FOUND
- Writing port 0x01 (ISR ACK `OUT`) — this has no effect on the FOUND latch

Because ARRIVE fires ~46× per ISR frame, FOUND is always cleared by the next scan cycle
well before the next 50 Hz ISR fires. The emulator approximates this by clearing the
`found` software flag after `keyboard_read_cla()` returns the key code, which is
equivalent in timing terms.
...
- **`found`** — "new key event" latch.  Set by `machine_inject_key()`.  Cleared by `keyboard_read_cla()` on the first call that returns a key code — approximating the hardware ARRIVE-based clear.
...
- **`cla_seen`** — ISR-cycle flag.  Set by every CLA read.  **Cleared by the ISR ACK** (`OUT (0x01), data≠0`) at the start of each 50 Hz frame.  This is a software-only emulator concept; there is no equivalent hardware signal.
