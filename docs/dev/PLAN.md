...
- **Physical keyboard delivery (post-boot)**: ISR Stage 2 is permanently blocked once SAMOS is running (`(0x4582)==0x80` sentinel). Physical keypresses are delivered via a software FIFO: `keyboard_event()` → `fifo[]` → `keyboard_frame_tick()` writes one key per frame directly to the SAMOS circular buffer at `0x4596`/ptr when the buffer is empty (ptr==0x4596). SDL key-repeat events are discarded.
