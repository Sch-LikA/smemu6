...
|-------|-----------|-------|-----------|-------------------------------------------------------------------------------|
| 0x00  | CLA       | R/W   | ✓ ROM/SYS | **Read**: keyboard CLA — current latched key code; bit 7 = 0 → key present, bit 7 = 1 → no key. Used only by the autoboot/inject path; physical keyboard uses a software FIFO that bypasses CLA entirely. **Write**: video-mode control — bit 0 = display enable, bit 2 = super mode, bit 3 = graphic mode; A=0x00 = display off. See `docs/dev/keyboard_analysis.md`. |
...
- **Physical keyboard delivery (post-boot)**: the emulator currently routes physical keypresses as `keyboard_event()` → 64-slot software FIFO → `keyboard_frame_tick()` → SAMOS circular buffer. That implementation remains in place because it matches the confirmed CLI blocking-read path.  A repo-local FIFO trace now also confirms this route bypasses `0x4581..0x4595` and writes the circular buffer directly.  Prompt-time RAM pokes then showed that pre-frame writes to `0x4580` are overwritten by `SYS.SY` itself, while `0x4582 = 0x80` still gates the `0x0179` debounce / second-CLA-read block.  A later `-fonct-on-prompt 0x01` probe proved that a real nonzero `0x4580` is enough to make Stage 3 / 4 toggle `0x458B` and advance the circular buffer.  A follow-up `-poke-at-pc 0x0179 0x4580 0x41` probe then showed that the promoted payload follows the live `0x4580` value itself, whereas the matching `0x4581` probe does not change the emitted byte.  A narrower `CLI.SY` trace now also confirms that this forced `0x41` reaches the normal visible line-editor insertion helper at `0x58DD..0x590F`, including the concrete prompt-buffer write `[45C0] '-' -> 'A'`.  The next probe tightened that again: overriding exactly one CLA read at `pc=0x0162` to `0xC1` makes `SYS.SY` itself seed `0x4580 = 0x41` and reproduce the same visible `A` path without any RAM poke.  A follow-up timing probe then forced the FOUND re-check at `pc=0x0180` high and confirmed that `SYS.SY` does take the second-read branch, but the actual second CLA read at `pc=0x0185` still returns `0x80`, so the emitted payload remains `0x01`.  The next direct probe then forced the second CLA read itself to `0x41`, which proved that this path can produce a later regular-key payload, but that later `0x41` remains hidden behind the first `0x01` item from `0x4580`.  Suppressing that first payload by poking `0x4580 = 0x00` at `pc=0x0179` makes the same second-read `0x41` become the first visible prompt insertion (`[45C0] = 'A'`).  The latest ordering traces sharpen that one more step: when the first payload survives, the re-entry after `0x0198` stores the regular-code `0x41` into `0x4581`; when the first payload is suppressed, the same path stores `0x41` back into `0x4580`.  Clearing `0x4580` only at `pc=0x0198` is still not enough, because the immediate `0x019B -> 0x01C9 -> 0x01DF` pass does not promote a lone `0x4581 = 0x41`.  Restoring `0x4580 = 0x41` exactly at the first `0x019B` walk *is* enough to make the first visible insertion become `A`.  The strongest negative result is earlier still: clearing `0x4580` immediately at the first `pc=0x0171` write is enough for the second-read re-entry to rewrite `0x4580` with `0x41`, while disabling the emulator's frame-time `fonct_bits -> 0x4580` mirror does not help because `SYS.SY` itself still seeds `0x4580 = 0x01` at `pc=0x0171`.  The strongest positive result now goes one step further: a one-shot model rule that makes the first CLA read of a held regular key return `0x80 | key_code` reproduces the visible `A` path with no PC-specific CLA override and no RAM poke, and that rule is now the default behavior for post-boot `-inject-keycode` / `machine_inject_key()` held keys.  The best current answer is therefore that original post-boot regular-key delivery likely presents a bit-7-set regular code to the first relevant CLA read, so Stage 1 stores the regular code into `0x4580` while still entering the Stage 2 / Stage 3 path.  The remaining implementation question is whether real physical SDL keypresses should also be routed through that same low-level CLA path instead of the current FIFO shortcut.
- **Autoboot / raw-key injection delivery**: `machine_inject_key()` drives the CLA-facing fields (`found`, `key_code`, `physically_held`) and now also applies the current best post-boot model: the first held regular-key CLA read returns `0x80 | key_code`, which makes `SYS.SY` seed `0x4580` with the regular code and reproduce the visible `A` path.  So the injected CLA path is now aligned with the best current hardware hypothesis.  The unresolved question is narrower: whether real physical SDL keypresses should also move from the current FIFO shortcut onto that same low-level producer path.
...
- **Full keyboard matrix audit**: the full S471 dump is now available and confirms four 64-entry layers (normal, Shift, FNCT/ALT, caps-like).  The emulator already matches the audited special codes it exposes directly, but most printable keys still follow host `SDL_TEXTINPUT` layout rather than a strict per-position Smaky matrix.  The strongest new architectural clue is that the older `0x04` / `0x05` / `0x07` trio belongs to a different key position near the Q-row right edge, not the top-left ESC / UNDO key.
