...
smemu6_bin="$1"
repo_root="$2"
tmp_dir="$repo_root/tmp/ctest-vfd-dx1-subdir"
...
SDL_VIDEODRIVER=dummy "$smemu6_bin" \
    ...
    -no-display-off \
    -scrdump \
    -inject-str 'TYPE DX1:BOX:INNER.BS\n' \
    -timeout 12 \
    > "$stdout_log" 2> "$stderr_log"

grep -F "mounted virtual host directory '$tmp_dir' on DX1" "$stderr_log" >/dev/null
grep -F "TYPE DX1:BOX:INNER.BS" "$stderr_log" >/dev/null
grep -F "INNERPATHOK" "$stderr_log" >/dev/null
