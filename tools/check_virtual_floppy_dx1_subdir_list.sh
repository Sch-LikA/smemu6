...
smemu6_bin="$1"
repo_root="$2"
tmp_dir="$repo_root/tmp/ctest-vfd-dx1-subdir-list"
...
grep -F "LIST DX1:BOX" "$stderr_log" >/dev/null
grep -F "INNER.BS" "$stderr_log" >/dev/null
grep -F "R p BOX" "$stderr_log" >/dev/null
