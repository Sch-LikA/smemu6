...
**Solution:** fix the whole chain, but keep the two paths separate: suppress the SDL text bypass, make the `0x0519` / `?GETFO` helper return held function bits directly, do not let `PROGRA` rewrite the ordinary matrix key through `S471_LAYER_FNCT`, and do not prefix the first ordinary CLA read as `0x80 | key_code`.

**Note for next time:** when a Smaky application combines a function key with an ordinary key, check the whole chain in order: host event path, matrix/text split, `?GETFO` accessor policy, host keyboard layout, whether the function key should modify the ordinary matrix byte at all, and whether ordinary CLA delivery is leaking into the function/no-key workspace path. The progression `z -> û -> ù -> û -> ordinary glyph` means multiple layers of compatibility logic are interfering with each other.
